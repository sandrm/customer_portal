...
- **[C-4] Test strategy is missing mandatory tooling details** — [AGENTS.md](cci:7://file:///D:/projects_2026/customer_portal/AGENTS.md:0:0-0:0) §4 requires explicit JUnit 5, Mockito, AssertJ, MockMvc, Testcontainers, and `@ServiceConnection`. The spec lists Gherkin and example method names but does not name the required frameworks.
...
- **[Fix C-4]**: Add a "Testing Tooling" subsection explicitly naming JUnit 5, Mockito, AssertJ, MockMvc, Testcontainers with `@ServiceConnection`, and the prohibition of H2.
